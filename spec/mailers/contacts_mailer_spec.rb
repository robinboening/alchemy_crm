...
describe AlchemyCrm::ContactsMailer do
	...
	before(:all) do
		...
		@subscription = AlchemyCrm::Subscription.create!(:contact => @contact, :newsletter => @newsletter, :verified => true, :wants => true)
	end
	...
	describe '#signup_mail' do
		...
		before(:all) do
			...
			@email = AlchemyCrm::ContactsMailer.signup_mail(@contact, @page).deliver
		end
	...
	describe '#signout_mail' do
		...
		before(:all) do
			...
			@element.content_by_name('text').essence.update_attribute(:body, 'Hello %{name}, please click %{link}')
			@email = AlchemyCrm::ContactsMailer.signout_mail(@contact, @page).deliver
		end
